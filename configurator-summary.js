...
(() => {
  ...
  const render = () => {
    ...
    summary.innerHTML = `<h3 id="configSummaryTitle">${labels[lang] || labels.en}</h3><p>${hint[lang] || hint.en}</p>${rows || `<span class="config-summary-empty">${T?.[lang]?.designerText || T?.en?.designerText || ''}</span>`}<button class="config-summary-reset" type="button">${reset[lang] || reset.en}</button>`;
